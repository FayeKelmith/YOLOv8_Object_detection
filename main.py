...
for result in results:
    ...
    #for rectangle on object
        cv2.rectangle(img,(int(cords[0]),int(cords[1])),(int(cords[2]),int(cords[3])),color=cls_id_col[cls],thickness=2)
    #text background color:
        text_size,baseline = cv2.getTextSize(name,cv2.FONT_HERSHEY_COMPLEX,fontScale=0.4,thickness=1)
        cv2.rectangle(img, (int(cords[0]), int(cords[1] - text_size[1] - baseline)), (int(cords[0] + text_size[0]), int(cords[1])), (50,50,50), -1)
    
    #to add text
        cv2.putText(img,name,(int(cords[0]),int(cords[1])),cv2.FONT_HERSHEY_COMPLEX,fontScale=0.4,color=(255,255,255),thickness=1)
